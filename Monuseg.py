...
class MonusegDataset(utils.Dataset): 
    ...
    def __init__(self, source = "GT", *args, **kwargs):
        self.source = source
        super(MonusegDataset, self).__init__(*args, **kwargs)
        
    def load_mask(self, image_id):
        # The field set in add_image (Contains: Source Class Name, ID, Path)
        info = self.image_info[image_id]       
        path = info["path"]
        # Navigate to binary masks
        ...
        # Binary task: Can return np.ones for class_id
    ...
    def load_image(self, image_id):
        ...
        if self.source == "GT":
            ...
            dist_path = os.path.join(os.path.dirname(os.path.dirname(path)), "distance_maps", name + "_gt.npy")
            dist = np.load(dist_path)  
            dist = (dist / np.max(dist) * 255).astype(np.uint8)                   
        elif self.source == "UNET":
            dist_path = os.path.join(os.path.dirname(os.path.dirname(path)), "distance_maps", name + "_unet.npy")        
            dist = np.load(dist_path)
            dist = (dist / np.max(dist) * 255).astype(np.uint8)          
        elif self.source == "ZEROS":
            dist = np.zeros((image.shape[0], image.shape[1]))       
        else:
            raise Exception("Unsupported Source argument")
...
def calc_and_save_detect(model, dataset, model_path):
    # Detect on dataset using model, save detections under model_path
