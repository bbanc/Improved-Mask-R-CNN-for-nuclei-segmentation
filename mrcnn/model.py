...
def identity_block(input_tensor, kernel_size, filters, stage, block,
                   use_bias=True, train_bn=True, dilation_rate=(1,1)):
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    x = KL.Conv2D(3, (1,1), name = "PREPROC")(x)
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # TODO TEST Increase box size to be at least equal to the avg box size
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    # BB Note: This can cause generators to die when used in a try/catch block in while loop: if len(gt_class_ids) is 0
...
class MaskRCNN():
    ...
    def set_log_dir(self, model_path=None):
        ...
        if model_path:
            ...
            if m:
                ...
                # Epoch number in file is 0-based, and in Keras code it's 0-based.
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        self.config.write_to_file(os.path.join(self.log_dir, "cfg.txt"))
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            # HOVER
            molded_image = mold_image(molded_image.astype(np.float32), self.config)
            # Build image_meta
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
                          image_shape, window, binarize = True):
        ...
        if binarize:
            ...
            for i in range(N):
                # Convert neural network mask to full size mask
                full_mask = utils.unmold_mask(masks[i], boxes[i], original_image_shape, binarize = False)
                full_masks.append(full_mask)
            full_masks = np.stack(full_masks, axis=-1)\
                if full_masks else np.empty(original_image_shape[:2] + (0,))           
    ...
    def detect(self, images, verbose=0, binarize = True):
        """Runs the detection pipeline.
            BB: Removes all small detections
        ...
        for i, image in enumerate(images):
            ...
            if binarize:
                ...
                remove_small_objects(final_masks, min_size=50, in_place=True)
            ...
            y2 = final_rois[:, 2]
            x2 = final_rois[:, 3]
            area_boxes = (y2 - y1) * (x2 - x1)
            
            logic = area_boxes < 100
            
            final_rois = np.delete(final_rois, logic, axis=0)
            final_masks = np.delete(final_masks, logic, axis=-1)
            final_scores = np.delete(final_scores, logic, axis=0)
            final_class_ids = np.delete(final_class_ids, logic, axis=0)
    ...
    def detect_all(self, images, verbose=0):
            """Runs the detection pipeline.
